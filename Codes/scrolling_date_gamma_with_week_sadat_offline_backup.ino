...
uint8_t N1 = 8; //digit size, sizeof(disp1[0]) / sizeof(disp1[0][0]);
...
//DATE AND TIME INFO FROM UTC
uint8_t tim_e[4] = {1, 0, 0, 7};
uint8_t ampm = 0; //0 if AM,1 if PM

uint8_t week  = 4; //Week day, Sat = 1

uint8_t bn_date[2] = { 2, 1 };
uint8_t bn_year[4] = { 1, 4, 2, 9 };
uint8_t bn_m = 9; //month

uint8_t en_date[2] = { 0, 5 };
...
uint64_t temp=0;
 ...
 if (dig_it==0){
  // ZERO
    ...
    else if(i==10)  temp = 0b00000010; //New
    else if(i==11)  temp = 0b00110100; //New
    else if(i==12)  temp = 0b01111000; //New
    else if(i==13)  temp = 0b00110000; //New
}
...
void populate_cells()
{
  ...
  uint8_t week_size = wk_size[week] + 3; //3 spaces after week
    ...
    uint8_t disp_size = ampm_size + week_size + d_size + bn_m_size[bn_m] + y_size + 3 + 3 + d_size + en_m_size[en_m] + y_size + 6;
  ...
  for(int i=0;i<16;i++){
    ...
    { 
      ...
      //ampm_size + week_size[week]+2 + d_size + bn_m_size[bn_m] + y_size + 3 + 3 + d_size + en_m_size[en_m] + y_size + 6
      ...
      else if (col_ind < (ampm_size + week_size + d_size + bn_m_size[bn_m]+y_size+6+d_size + en_m_size[en_m] + y_size))
